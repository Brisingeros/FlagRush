...
public class RotateMoveCamera : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        { // al precionar la tecla.W))
            transform.Translate(new Vector3(0, 0, velocity)); //cambiar posision.trasladar (aun nuevo vector(usando estas codenadas)
        }
        else {
        ...
        {
            transform.Translate(new Vector3(velocity, 0, 0)); //cambiar posision.trasladar (aun nuevo vector(usando estas codenadas)
        }
        else {
